...
public:
    ...
    void dfs(int q, vector<int> &canon, int &qCount)
    {
        canon[q] = qCount;
        qCount += 1;
        for (size_t xIndex = 0; xIndex < this->m; xIndex++)
            if (canon[this->D[q][xIndex]] == -1)
                dfs(this->D[q][xIndex], canon, qCount);
    }
    void canonic()
    {
...
int main()
{
    ...
    MealyMachine<int, string> MM1(n, m, q0);
    for (auto &&i : MM1.D)
        for (auto &&j : i)
            cin >> j;
    ...
    MealyMachine<int, string> MM2(n, m, q0);
    for (auto &&i : MM2.D)
        for (auto &&j : i)
            cin >> j;
    ...
    MealyMachine<int, string> MMM1 = MM1.AufenkampHohn();
    MMM1.canonic();

    MealyMachine<int, string> MMM2 = MM2.AufenkampHohn();
    MMM2.canonic();

    if (MMM1.n != MMM2.n || MMM1.m != MMM2.m)
    {
        cout << "NOT EQUAL" << endl;
        return 0;
    }

    for (size_t i = 0; i < MMM1.n; i++)
    {
        for (size_t j = 0; j < MMM1.m; j++)
        {
            if (MMM1.canonD[i][j] != MMM2.canonD[i][j] || MMM1.canonF[i][j] != MMM2.canonF[i][j])
            {
                cout << "NOT EQUAL" << endl;
                return 0;
            }
