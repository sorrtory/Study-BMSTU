...
def compile_display_lists():
    ...
    """
    global Torus_DL, Bounds_DL, Axes_DL, Light_Point_DL
    ...
    Bounds_DL = glGenLists(1)
    glNewList(Bounds_DL, GL_COMPILE)
    draw_bounds()
    glEndList()
    ...
    # Unused
    # Compile light marker into a display list
    Light_Point_DL = glGenLists(1)
    glNewList(Light_Point_DL, GL_COMPILE)
    draw_light_marker() 
    glEndList()
...
def display(window):
    ...
    if Torus_use_texture and texture_id:
...
def key_callback(window, key, scancode, action, mods):
    ...
    global Torus_wirerame, Torus_use_texture, animation, Torus_Velocity
    global light_index, material_index, Axes_show, Bounds_show
    global scene_Angle, Light_Enabled
    if action == glfw.PRESS or action == glfw.REPEAT:
        ...
        # ----- Polygon toggle ----
            ...
            Torus_wirerame = not Torus_wirerame
            glPolygonMode(GL_FRONT_AND_BACK, GL_LINE if Torus_wirerame else GL_FILL)

        # ----- Texture toggle ----
        elif key == glfw.KEY_T:
            Torus_use_texture = not Torus_use_texture
        ...
        # ----- Toggle bounds display ----
            ...
            Bounds_show = not Bounds_show

        # ----- Toggle axes display ----
        elif key == glfw.KEY_V:
            Axes_show = not Axes_show
