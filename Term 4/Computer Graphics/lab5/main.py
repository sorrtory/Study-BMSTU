...
window = glfw.create_window(
    width, height, "3D Cyrus-Beck Clipping", None, None)
if not window:
    glfw.terminate()
...
def cyrus_beck_clip(P0, P1):
    # Define planes with correct distances (d) for a cube centered at the origin
    planes = [
    ...
    t_enter = 0.0
    t_exit = 1.0
    D = P1 - P0  # Direction vector of the line

    for normal, d in planes:
        numerator = d - np.dot(normal, P0)
        denominator = np.dot(normal, D)

        if abs(denominator) < 1e-6:  # Line is parallel to the plane
            if numerator < 0:  # Line is outside the plane
                return None
        else:
            t = numerator / denominator
            if denominator < 0:  # Entering the plane
                t_enter = max(t_enter, t)
            else:  # Exiting the plane
                t_exit = min(t_exit, t)

            if t_enter > t_exit:  # No intersection
                return None

    # Calculate the clipped points if valid t values exist
    if 0 <= t_enter <= 1 and 0 <= t_exit <= 1:
...
def display():
    ...
    if clipped:
        ...
        glBegin(GL_POINTS)
        glColor3f(1, 1, 0)
        glVertex3fv(clipped[0])  # Yellow entry point
        glColor3f(0, 1, 1)
        glVertex3fv(clipped[1])  # Cyan exit point
        glEnd()
