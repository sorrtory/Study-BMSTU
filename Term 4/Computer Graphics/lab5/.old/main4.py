...
def display():
    ...
    if clipped:
        draw_line(clipped[0], clipped[1], (0, 1, 0), 3.0)
        glPointSize(8.0)
        glBegin(GL_POINTS)
        glColor3f(1,1,0); glVertex3fv(clipped[0])
        glColor3f(0,1,1); glVertex3fv(clipped[1])
        glEnd()
    
    glfw.swap_buffers(window)

def key_callback(window, key, scancode, action, mods):
    global rotation_x, rotation_y, zoom, line_start, line_end
