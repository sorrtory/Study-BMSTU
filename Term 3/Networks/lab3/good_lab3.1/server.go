...
func (client *Client) handleRequest(req *Request) bool {
	sendToWebForm("Server command: " + req.Command, "SSE")
	switch req.Command {
	case "quit":
	...
	case "getValue":
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &s); err != nil {
				...
				sendToWebForm(fmt.Sprintf("Geting value on to %d index", s.StartIndex), "SSE")
			}
		}
	...
	case "setValue":
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &s); err != nil {
				...
				sendToWebForm(fmt.Sprintf("Updating value on %d index to %d", s.EndIndex, s.N), "SSE")
			}
		}
	...
	case "giveRequestedValue":
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &s); err != nil {
				...
				sendToWebForm(fmt.Sprintf("Give value %d", s.N), "SSE")
			}
		}
