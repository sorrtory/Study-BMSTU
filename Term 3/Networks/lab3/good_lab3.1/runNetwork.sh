...
src=($SCRIPT_DIR/test.txt $SCRIPT_DIR/main.go) # Files to upload
path=/root/test/lab # Path to lab folder on server
startCommand="go run main.go" # Launch command 
NetworkList=("yss1" "yss2" "yss3" "yss4") # List of ssh hosts (supposed to use ssh key)

# Options
clearMode=false # Remove lab dirs and exit
forceClear=false # Don't ask for confirmation before replacing lab files
justStart=false # Don't upload files, just execute $startCommand on server in tmux
IHAVESSHKEY=false # Set false and set IPS and PASSWORDS to generate ssh keys
