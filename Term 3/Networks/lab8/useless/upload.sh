...
src=("$SCRIPT_DIR/*.go" "$SCRIPT_DIR/key.json") # Files to upload
path=/root/Fedukov/lab8/ # Path to lab folder on server
...
justStart=true # Don't upload files, just execute $startCommand on server in tmux
