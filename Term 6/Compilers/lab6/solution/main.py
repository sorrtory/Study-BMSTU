...
class PredictiveParser:
    ...
    def _build_table(self) -> None:
        ...
        для каждого токена a из FIRST(α):
            table[A, a] = α

        Если α может быть пустым, то есть:

        ε ∈ FIRST(α)

        тогда:

        для каждого токена b из FOLLOW(A):
            table[A, b] = α

        В лекциях это и есть алгоритм построения таблицы предсказывающего анализатора: 
        - сначала все ячейки считаются ошибками
        - потом для каждого правила ячейки заполняются по FIRST правой части
        - а если правая часть может быть ε — ещё и по FOLLOW левой части
        """
        T = TokenType
