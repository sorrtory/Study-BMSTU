...
> На основе лабораторной работой 2.2
...
- Имена констант и типов находятся в общей области видимости и не должны
  повторяться. Перечислимые типы тоже определяют константы.
...
- Для каждого типа должен вычисляться его объём. Считаем, что размеры целых
  чисел и перечислимых типов — 2 байта, вещественных чисел — 4 байта, размер
  указателя — 4 байта, размер множества определяется как количество байт,
  требуемых для его представления (каждый элемент множества — бит), размер
  case-части записи определяется как размер поля тега + размер наибольшего
  варианта.
...
Типы:
  COORDS = 4 байт
  COORDSVECTOR = 400 байт
  BASECOLOR = 2 байт
  COLOR = 1 байт
  GRAPHICSCREEN = 307200 байт
  TEXTSCREEN = 5760 байт
  DOMAIN = 2 байт
  TOKEN = 42 байт
  YEAR = 2 байт
  LIST = 46 байт
...
В ходе работы был реализован семантический анализ объявлений типов и констант
Паскаля. Анализатор проверяет порядок объявлений, повторные имена и поля
записей, а также вычисляет значения констант и размеры типов.
